-- Adds farm-specific areas and customer farm-area assignments.
-- Corrected version for PostgreSQL UPDATE scoping.
...
create trigger validate_area_sales_prices_farm
before insert or update of area_id, product_id on public.area_sales_prices
for each row execute function public.validate_area_sales_price_farm();

create index if not exists idx_customer_areas_farm
on public.customer_areas(farm_id)
...
-- Backfill customer farm-area assignments when old customer.area_id exists
-- and that area has already been assigned to a farm.
insert into public.customer_farm_areas (customer_id, farm_id, area_id)
select c.id, a.farm_id, a.id
from public.customers c
join public.customer_areas a on a.id = c.area_id
where c.deleted_at is null
  and a.deleted_at is null
  and a.farm_id is not null
on conflict (customer_id, farm_id, area_id)
do update set
  deleted_at = null,
  updated_at = now();

-- Backfill daily order item area snapshots where possible.
-- Fixed: avoids referencing target table alias inside FROM JOIN condition.
update public.daily_order_items doi
set area_id = coalesce(
  (
    select cfa.area_id
    from public.customer_farm_areas cfa
    where cfa.customer_id = doi.customer_id
...
from public.customers c
where doi.customer_id = c.id
  and doi.area_id is null;
